...
class OrderTest {
    ...
    void constructorThrowsExceptionIfOrderIdIsNull() {
        assertThrows(NullPointerException.class,
            () -> new Order(null, "cust-123", "SKU-001", 5));
    }
    ...
    @Test
    void constructorThrowsExceptionIfCustomerIdIsBlank() {
        assertThrows(IllegalArgumentException.class,
            () -> new Order(UUID.randomUUID(), "   ", "SKU-001", 5));
    }

    @Test
    void constructorThrowsExceptionIfItemIdIsNull() {
        assertThrows(IllegalArgumentException.class,
            () -> new Order(UUID.randomUUID(), "cust-123", null, 5));
    }

    @Test
    void constructorThrowsExceptionIfItemIdIsBlank() {
        assertThrows(IllegalArgumentException.class,
            () -> new Order(UUID.randomUUID(), "cust-123", "   ", 5));
    }

    @Test
    void constructorThrowsExceptionIfQuantityIsZero() {
        assertThrows(IllegalArgumentException.class,
            () -> new Order(UUID.randomUUID(), "cust-123", "SKU-001", 0));
    }

    @Test
    void constructorThrowsExceptionIfQuantityIsNegative() {
        assertThrows(IllegalArgumentException.class,
            () -> new Order(UUID.randomUUID(), "cust-123", "SKU-001", -5));
    }

    @Test
    void markReservedTransitionsFromPendingToReserved() {
        Order order = new Order(UUID.randomUUID(), "cust-123", "SKU-001", 5);
    ...
    void equalsBasedOnOrderId() {
        UUID orderId = UUID.randomUUID();
        Order order1 = new Order(orderId, "cust-123", "SKU-001", 5);
    ...
    void notEqualsForDifferentOrderIds() {
        Order order1 = new Order(UUID.randomUUID(), "cust-123", "SKU-001", 5);
        Order order2 = new Order(UUID.randomUUID(), "cust-123", "SKU-001", 5);
